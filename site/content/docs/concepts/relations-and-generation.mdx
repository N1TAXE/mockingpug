...
If you need several fields pulled from the *same* related record, there's
no dedicated syntax for that yet; the current workaround is to only
reference the id (`"data.product.id"`) and resolve the rest with an extra
lookup at read/request time in your app, or duplicate the value into a
custom dictionary keyed the way you need it. Track the roadmap if you need
a native multi-field pick.
</Callout>
