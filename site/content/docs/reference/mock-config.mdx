...
lock you out of the very page that renders the panel: if the page does any
server-side data fetching and your app has no error boundary around it, the
whole route fails to render, `<MockDevtools>` never mounts, and there's no
UI left to dial `errorRate` back down. If this happens, just **restart the
dev server** (or redeploy) — `ctx.runtime` is in-memory only, so the next
...
rendering regardless of `errorRate`.
