...
| `MP-GEN-001` | A `crossRef` field reached the low-level value generator directly instead of going through the dependency-graph resolver. This is user-reachable today via `array[data.<entity>...].N` — a `crossRef` as an array's inner type isn't supported yet (see [Schema DSL → Arrays](/docs/concepts/schema-dsl#arrays)); everywhere else it's an internal invariant violation, report it as a bug. |
