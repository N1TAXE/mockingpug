...
// relation (`blogpost.author` -> user.id), GET by id, POST create, search.
function App() {
  ...
  return (
    <div id="app">
      <h1>mockingpug + Vite example</h1>
      <p>
        Every request below hits real <code>fetch('/api/user')</code> calls, intercepted by MSW
        with data generated from <code>mock/api/user/schema.json</code> +{' '}
        <code>mock/api/blogpost/schema.json</code>. Open devtools' Network tab — the requests are
